...
//                            ->where('misc->entity_id', $event->id)->first();
    ...
    public function myCalendar(Request $request): RedirectResponse|InertiaResponse
    {
        $client_id = request()->user()->client_id;
    ...
    public function quickView(Request $request): RedirectResponse|InertiaResponse
    {
        $client_id = request()->user()->client_id;
        if ($client_id === null) {
            return Redirect::route('dashboard');
        ...
        if (! $request->has('start')) {
            $request->merge([
                'start' => date('Y-m-d H:i:s', DateTime::createFromFormat('Y-m-d H:i:s', (new DateTime())->format('Y-m-d 00:00:00'))
                    ->getTimestamp()),
            ]);
            $request->merge([
                'end' => date('Y-m-d H:i:s', DateTime::createFromFormat('Y-m-d H:i:s', (new DateTime())->format('Y-m-d 23:59:59'))
                    ->getTimestamp()),
            ]);
        } else {
            $date = date('Y-m-d', strtotime($request->get('start')));
            $request->merge([
                'start' => date('Y-m-d H:i:s', DateTime::createFromFormat('Y-m-d H:i:s', (new DateTime())->format($date . ' 00:00:00'))
                    ->getTimestamp()),
            ]);
            $request->merge([
                'end' => date('Y-m-d H:i:s', DateTime::createFromFormat('Y-m-d H:i:s', (new DateTime())->format($date . ' 23:59:59'))
                    ->getTimestamp()),
            ]);
        }
    ...
    public function eventTypes(Request $_): InertiaResponse
    {
        return Inertia::render('Calendar/EventTypes/Show');
    }

    public function createEventType(Request $_): InertiaResponse
    {
        return Inertia::render('Calendar/EventTypes/Create');
    }

    public function editEventType(Request $_, CalendarEventType $calendarEventType): InertiaResponse
    {
