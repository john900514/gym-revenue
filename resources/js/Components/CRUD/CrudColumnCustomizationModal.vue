<template>
    <daisy-modal v-bind="$attrs" :id="`${modelKey}-edit-columns`" ref="customizationModal">
        <h1 class="text-lg">{{ modelName }} CRUD Customization</h1>
        <div class="flex flex-col gap-2">
            <p class="text-base-content text-sm text-opacity-50">Edit Columns</p>
            <div class="form-control flex flex-row gap-2" v-for="field in fields">
                <input :id="field.name" type="checkbox" :value="field.name" v-model="form.columns"/> <label
                :for="field.name">{{ field.label }}</label>
            </div>
            <button type="button" @click="handleSubmit" class="btn btn-primary mt-4">Save</button>
        </div>
    </daisy-modal>
</template>
...
export default defineComponent({
    ...
    setup(props, {}) {
        ...
        const handleSubmit = () => form.post(route('crud-customize'), {onSuccess: close});
