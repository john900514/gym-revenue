<template>
    <header class="hidden lg:flex justify-between items-center py-4 px-6 bg-base-300 border-b border-base-100">
        ...
        <div class="flex items-center">
            <!-- Notifications -->
            <div class="relative">
                <button @click="showingNotificationDropdown = ! showingNotificationDropdown" class="flex mx-4  focus:outline-none">
            ...
            <div class="relative">
                ...
                <span v-else @click="showingUserDropdown = ! showingUserDropdown" class="inline-flex rounded-md">
                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md  hover: focus:outline-none transition ease-in-out duration-150">
                ...
                <div v-show="showingUserDropdown"
                    ...
                    <inertia-link :href="route('profile.show')" class="block px-4 py-2 text-sm  hover:bg-base-100">Profile</inertia-link>

                    <inertia-link :href="route('api-tokens.index')" v-if="$page.props.jetstream.hasApiFeatures" class="block px-4 py-2 text-sm  hover:bg-base-100">API Tokens</inertia-link>

                    <form @submit.prevent="logout">
                        <button class="block px-4 py-2 text-sm  hover:bg-base-100 w-full text-left focus:outline-none">
                            Log Out
                        </button>
                    </form>
                </div>
