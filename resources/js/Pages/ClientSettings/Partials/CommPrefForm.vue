<template>
    <jet-form-section @submitted="handleSubmit" collapsable>
        ...
        <template #form>
            ...
            <div class="text-xs text-warning col-span-6 sm:col-span-4 flex-row" style="display:ruby">
                <template v-if="hasTwilioSetup">
                    Conversation and Text are a bit similar, the difference is that conversation allows a continues interaction between you and the
                    lead/member, while text is a one way event.
                </template>
                <template v-else>
                    SMS and Conversation requires twilio gateway data (
                    <b>Twilio SID</b>, <b>Twilio Token</b> and <b>Twilio Number</b>) to work.
                </template>
            </div>
        </template>
...
const hasTwilioSetup = computed(() => {
    ...
    return ['twilioSID', 'twilioToken', 'twilioNumber'].every(name => gatewayNames.includes(name));
});

console.log(222, hasTwilioSetup.value)

const form = useGymRevForm({
    [COMMUNICATION_TYPES_CONVERSATION]: props.commPreferences[COMMUNICATION_TYPES_CONVERSATION],
    [COMMUNICATION_TYPES_SMS]: props.commPreferences[COMMUNICATION_TYPES_SMS],
    [COMMUNICATION_TYPES_VOICE]: props.commPreferences[COMMUNICATION_TYPES_VOICE],
    [COMMUNICATION_TYPES_EMAIL]: props.commPreferences[COMMUNICATION_TYPES_EMAIL],
});
...
function preventSelection(value) {
    return (value === COMMUNICATION_TYPES_CONVERSATION && (! hasTwilioSetup.value || form[COMMUNICATION_TYPES_SMS]))
    || (value === COMMUNICATION_TYPES_SMS && (! hasTwilioSetup.value || form[COMMUNICATION_TYPES_CONVERSATION]))
}
</script>
