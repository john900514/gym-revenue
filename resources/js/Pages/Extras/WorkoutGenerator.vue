...
<script>
    ...
    export default defineComponent({
        ...
        watch: {
            ...
            plans(workout) {
                if(workout !== '') {
                    console.log('Setting up the workout solution for the GeneratedWorkout to react to', workout)
        ...
        data() {
            return {
                ...
                injuryExclusions: {
                    ...
                    neck:{'BB Squat':'', 'BB Squat to High Pull':'', 'BB Squat to Press':'',
                        'Overhead Press':'', 'Upright Rows':'', 'Shrugs':''},

                    back:{'BB Squat':'', 'BB Squat to High Pull':'', 'BB Squat to Press':'',
                        'Front Squat':'', 'Deadlifts':''},

                    hip:[],

                    knee:{'HIIT Lateral Movements':'', 'Step Ups':'',
            ...
            getSchedule() {
                ...
                switch(parseInt(this.rest)) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        results = {'Monday':'', 'Wednesday':'', 'Friday':'', 'Saturday':''};
                        break;
                    default:
                }
        ...
        methods: {
            ...
            getLower(df, day, nMax) {
                ...
                let dfMajSize = 0;
                let dfMinSize = 0;
                for(let x in dfMajor['Exercise']) { dfMajSize ++; }
                for(let x in dfMinor['Exercise']) { dfMinSize ++; }
            ...
            getCore(df, n) {
                let dfCoreSize = 0;
                for(let x in df['Exercise']) { dfCoreSize ++; }
            ...
            getUpper(df, day, nGrp, nMax) {
                ...
                for(let group in groups) {
                    let dfGroupMinor = this.filterValidValues(dfMinor, 'Muscle Group', group);
                    if(group in major_groups) {
                        let dfGroupMajor = this.filterValidValues(dfMajor, 'Muscle Group', group);

                        let dfMajSize = 0;
                        let dfMinSize = 0;
                        for(let x in dfGroupMajor['Exercise']) { dfMajSize ++; }
                        for(let x in dfGroupMinor['Exercise']) { dfMinSize ++; }
                        ...
                        dfGroupMajor = this.filterSampleValues(dfGroupMajor, sampleMaj);
                        dfGroupMinor = this.filterSampleValues(dfGroupMinor, sampleMin);
            ...
            planWeek() {
                ...
                // sort if bodyband
                if(this.bodyBand) {
                    dfLower = this.filterValidValues(dfLower, 'Body or Band', 'True');
                    dfUpper = this.filterValidValues(dfUpper, 'Body or Band', 'True');
                    dfCore = this.filterValidValues(dfCore, 'Body or Band', 'True');
                }

                // sort if injury
                if((this.injury !== '') && (this.injury !== 'none')) {
                    console.log('Applying injury considerations', {'injury': this.injury, 'exclusions': this.injuryExclusions[this.injury]})
        ...
        mounted() {
            console.log('WorkoutGeneratorContainer mounted!', {'core' : this.core, 'upper': this.upper, 'lower': this.lower});
